...
class XDataTable(DataTable[str | Text]):
    DEFAULT_CSS = """\
XDataTable {
    &.empty {
        display: none;
...
"""
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        """The cursor can escape the table by pressing up when it's at the top (will focus previous widgets in focus chain)."""
    ...
    def toggle_row(self, row_key: RowKey) -> None:
        try:
            checkbox: XDataTable.Checkbox = self.rows[row_key].label
        except KeyError:
            return
        else:
            checkbox.toggle()
