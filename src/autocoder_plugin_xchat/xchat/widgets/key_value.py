...
from .datatable import XDataTable


class XKeyValueInput(Horizontal):
    @dataclass
    class Change(Message):
        key: str
        value: str
        _control: "XKeyValueInput"

        @property
        def control(self) -> "XKeyValueInput":
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="row-writer-footer"):
            yield Label(
                "正在编辑行. 按 [i]escape[/i] 取消.", id="editing-row-label"
            )
...
class KeyValueEditor(Vertical):
    ...
    def __init__(
        self,
        table: XDataTable,
        key_value_input: XKeyValueInput,
    ...
    @on(XKeyValueInput.Change)
    def add_key_value_pair(self, event: XKeyValueInput.Change) -> None:
        if self._row_being_edited is None:
            # Adding a new row.
            table = self.table
    ...
    @on(XDataTable.RowsRemoved)
    def rows_removed(self, event: XDataTable.RowsRemoved) -> None:
    ...
    @on(XDataTable.RowsAdded)
    def rows_added(self, event: XDataTable.RowsAdded) -> None:
        rows = event.data_table.row_count
        self.set_class(rows == 0, "empty")

    @on(XDataTable.RowSelected)
    def row_selected(self, event: XDataTable.RowSelected) -> None:
    ...
    def enter_edit_mode(self, row_key: RowKey, focus_value: bool = False) -> None:
        ...
        # input widgets (this will update the background color and button label to indicate that we are
        # in edit mode).
